"""
This class receives input from the user: It captures audio data from the microphone in real-time using pyaudio.
The microphone can be suppressed and resumed based on the mic_on_at timestamp, which controls when the microphone is active or inactive.
Audio data is stored in a buffer for both the microphone (input) and the speaker (output), ensuring handling of audio streams without glitches.
on_audio_callback allows real-time audio data to be handled. ALLOWS US TO HANDLE CAPTURED DATA -> PUSH TO API? 
receive_audio appends all received audio chunks to be played through speakers (stream playback audio)
"""
import pyaudio
import queue
import time
